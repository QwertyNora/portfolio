...
const BrowserCard = ({ filename, children, dotSize = "sm", hoverable = false }: BrowserCardProps) => {
    ...
    return (
        <div
            className={`bg-(--bg-secondary) border border-(--border) rounded-[10px] shadow-sm overflow-hidden transition-colors duration-200 ${hoverable ? "hover:shadow-md cursor-pointer" : ""}`}
        >
